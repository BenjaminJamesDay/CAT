...
class ConditionalAttentionMech(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, leak, condition=True):
        ...
        self.a_i = nn.Parameter(nn.init.xavier_uniform(torch.Tensor(2*out_features, 1).type(t_type),
                                                       gain=np.sqrt(2.0)),
                                requires_grad=True)
        self.a_j = nn.Parameter(nn.init.xavier_uniform(torch.Tensor(2*out_features, 1).type(t_type),
                                                       gain=np.sqrt(2.0)),
                                requires_grad=True)
        
        # conditioning parameter vectors
        self.w_gamma = nn.Parameter(nn.init.xavier_uniform(torch.Tensor(2*out_features, 1).type(t_type),
                                                           gain=np.sqrt(2.0)),
                                    requires_grad=True)
        
        self.w_beta = nn.Parameter(nn.init.xavier_uniform(torch.Tensor(2*out_features, 1).type(t_type),
                                                          gain=np.sqrt(2.0)),
                                   requires_grad=True)

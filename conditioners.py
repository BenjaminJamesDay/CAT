...
class coraConditionerModel3(nn.Module):
    def __init__(self):
        super(coraConditionerModel3, self).__init__()
        self.hidden1 = nn.Linear(1433,64)
        self.hidden2 = nn.Linear(64,32)
        self.hidden3 = nn.Linear(32,40)
...
def coraConditioner3(x):
    ...
    model = coraConditionerModel3()
    model.cuda()
    cond = torch.cat([model(example) for example in x])
    #reshape the concatenated set (2N.N_mechs) to (N*2*N_mechs) then permute to (2*N_mechs*N)
    return cond.view(2708,2,20).permute(1,2,0)

class coraConditionerModel4(nn.Module):
    def __init__(self):
        super(coraConditionerModel4, self).__init__()
        self.hidden1 = nn.Linear(1433,64)
        self.hidden2 = nn.Linear(64,32)
...
def coraConditioner4(x):
    ...
    model = coraConditionerModel4()
    model.cuda()
    cond = torch.cat([model(example) for example in x])
    #reshape the concatenated set (2N.N_mechs) to (N*2*N_mechs) then permute to (2*N_mechs*N)
    return cond.view(2708,2,8).permute(1,2,0)

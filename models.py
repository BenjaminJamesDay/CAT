...
class CCModel2(nn.Module):
    ...
    """
    def __init__(self, ins, classes, conditioner):
        super(CCModel2, self).__init__()
        
        # dropout is included in the layers so we don't need to add anything else
        # activate the first layer and use the automatic ELU
        self.CAT1 = ConditionalAttentionLayer(ins=ins, outs=8, dropout=0.2, leak=0.1, N_mechs=16,
                                              conditioner=conditioner, activate=True)
        # do not activate the output
        self.CAT2 = UnconditionalAttentionLayer(N_mechs=2, dropout=0.4, ins=128, leak=0.1, outs=classes, concat=False)
...
class CCModel3(nn.Module):
    ...
    """
    def __init__(self, ins, classes, conditioner):
        super(CCModel3, self).__init__()
        
        # dropout is included in the layers so we don't need to add anything else
        # activate the first layer and use the automatic ELU
        self.CAT1 = ConditionalAttentionLayer(ins=ins, outs=5, dropout=0.2, leak=0.1, N_mechs=20,
                                              conditioner=conditioner, activate=True)
        # do not activate the output
        self.CAT2 = UnconditionalAttentionLayer(N_mechs=2, dropout=0.4, ins=100, leak=0.1, outs=classes, concat=False)
...
class CCModel4(nn.Module):
    ...
    """
    def __init__(self, ins, classes, conditioner):
        super(CCModel4, self).__init__()
        
        # dropout is included in the layers so we don't need to add anything else
        # activate the first layer and use the automatic ELU
        self.CAT1 = ConditionalAttentionLayer(ins=ins, outs=16, dropout=0.2, leak=0.1, N_mechs=8,
                                              conditioner=conditioner, activate=True)
        # do not activate the output
        self.CAT2 = UnconditionalAttentionLayer(N_mechs=2, dropout=0.4, ins=128, leak=0.1, outs=classes, concat=False)
